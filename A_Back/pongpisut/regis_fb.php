...
if (isset($_GET['INPUT_FBNAME']) || isset($_GET['INPUT_FBEMAIL']) || isset($_GET['INPUT_FBID']) || isset($_GET['INPUT_PW']) || isset($_GET['INPUT_FBCONFIRM'])) {
    ...
    if (!isset($_SESSION["hdnCountFriends"])) {
        exit('<script>swal( "แจ้งเตือน", "ระบบไม่สามารถรับข้อมูลจำนวนเพื่อนของคุณได้ >> จำนวนเพื่อนของคุณต้องมีอย่างน้อย ' . $config['registerfb_friends'] . ' คนขึ้นไป", "error" )</script>');
    }
    if ($count_wzfb >= 1 || $count_acfb >= 1) {
        exit('<script>sweetAlert( "ล้มเหลว", "ไอดีเฟสบุ๊คหรืออีเมลนี้ถูกสมัครใช้งานในระบบของเราแล้ว", "error" ).then((value) => {
...
?>
                        <form class="navbar-login-form" method="get" autocomplete="off">
                            ...
                            <center>
                                ...
                                <a href="https://www.facebook.com/<?php echo $_SESSION['fb_id']; ?>" target="_blank" role="button" class="btn btn-default">เข้าสู่หน้าโปรไฟล์</a>    
                                <a href="index.php?do=logout_fb" class="btn btn-danger" role="button">ออกจากระบบ</a><br>
                                <div>
                                <p>ชื่อผู้ใช้ Facebook :</p>
                                <div class="youplay-input">
                                    <input type="text" name="FB_NAME" id="FB_NAME" value="<?php echo $_SESSION['fb_name']; ?>" class="form-control" disabled/>
                                </div>
                                ...
                                <p>รหัสผ่าน:</p>
                                <div class="youplay-input">
                                    <input type='password' class="form-control" name="pw" id="pw" class="form-control" maxlength="32" required/>
                                </div>

                                <p>ยืนยันรหัสผ่าน:</p>
                                <div class="youplay-input">
                                    <input type='password' class="form-control" name="confirm_pw" id="confirm_pw" class="form-control" maxlength="32" required/>
                                </div>
